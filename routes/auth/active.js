...
router.get('/', async (req, res) => {
    ...
    if (_user === null || _user === undefined)
        return await responseFunction(res, 409, 'ERR_USER_NOT_FOUND');
    ...
    if (!_token)
        return await responseFunction(res, 409, 'ERR_PROVIDED_TOKEN_INVALID');
    ...
    const _verify = await User.updateOne(
        { 'account.email': email, 'account.status': 'unknown' },
    ...
    return await SAVE_LOG(
        await responseFunction(res, 200, 'SUCCEED_USER_ACTIVATED')
    );
