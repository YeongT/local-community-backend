...
const jwtgetUser = async (req, jwtToken, options) => {
    var _response = {
        jwtbody: 'ERR_SERVER_FAILED_TEMPORARILY',
        jwtuser: null,
        jwterror: 'ERR_JWT_GET_USER_SYNTAX_FAILED',
    };
    const jwtUserResponse = (body, user, error) => {
        if (!(body && user !== undefined))
            throw 'ERR_JWT_GET_USER_SYNTAX_FAILED';
    ...
    if (tokenerror !== null)
        return jwtUserResponse(
            'ERR_JWT_AUTHENTICATION_FAILED',
            null,
            tokenerror
        );
    if (jwtdecode.decodable)
        return jwtUserResponse(
            'ERR_JWT_AUTHENTICATION_FAILED',
            null,
            'JWT_REFRESH_TOKEN_ACCESS_DENIED'
        );
    ...
    const _user = await User.findOne(
        {
            'account.email': jwtdecode.account.email,
    ...
    if (_user === null || _user === undefined)
        return jwtUserResponse(
            ...
            null,
            'JWT_USER_ACCOUNT_NOT_ACCESSIBLE'
        );
    if (!_user)
        return jwtUserResponse('ERR_JWT_AUTHENTIFCATION_FAILED', null, _user);

    //#VALIDATE USER COMMUNITY PRIVILEGE WHEN OPTIONS WERE PROVIDED
    if (!(options === undefined || options === null)) {
        const { permissions, verify_error } = await jwtVerifyAccess(
            jwtdecode.account._id,
            options.type,
            options.target
        );
    ...
    return await SAVE_LOG(
        jwtUserResponse('SUCCEED_JWT_AUTH_GRANTED', _user, null)
    );
};
