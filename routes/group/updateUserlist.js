...
const updateUserlist = async (community, userlist, userid, role) => {
    if (!(community && userlist !== undefined && userid && role))
        throw 'ERR_UPDATE_USERLIST_DATA_NOT_PROVIDED';
    ...
    const { privileges, privilege_error } = await genPrivilege(
        userid,
        community,
        role
    );
    if (privilege_error !== null)
        return { userlist: null, listerror: privilege_error };
