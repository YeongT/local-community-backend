...
router.put ("/", async (req,res) => {
    ...
    if (!(groupname && description && areaString && tags)) return await responseFunction(res, 412, "ERR_DATA_NOT_PROVIDED", null);
    ...
    const { privileges, userlist, listerror } = await buildUserlist(createCommunity._id, null, jwtuser._id, ["owner","admin","member"]);
    if (listerror !== null) return await SAVE_LOG(await responseFunction(res, 500, "ERR_COMMUNITY_CREATE_FAILED", null, listerror));
    createCommunity.userlist = userlist;

    //#UPDATE USER COMMUNITY LIST USING PREVIOUS QUERY
    const _update = await updateGrouplist(jwtuser._id, createCommunity._id, privileges);
    if (_update !== null) return await SAVE_LOG(await responseFunction(res, 500, "ERR_COMMUNITY_UPDATE_FAILED", null, _update));
    ...
    if (!_result) {
        ...
        await User.UpdateOne({"account.email": jwtuser.account.email}, {"service.community": jwtuser.service.community});
        return await SAVE_LOG(await responseFunction(res, 500, "ERR_COMMUNITY_UPDATE_FAILED", null, _result), createCommunity);
    }
    return await SAVE_LOG(await responseFunction(res, 200, "SUCCEED_COMMUNITY_CREATED", {"community":createCommunity._id}, null), createCommunity);
});
