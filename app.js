...
try {
    ...
    const db_connect = () => {
        const mongouri = `mongodb://${process.env.DB_USER}:${qs.escape(
            process.env.DB_PASSWORD
        )}@${process.env.DB_HOST}:${process.env.DB_PORT}/${
            process.env.DB_NAME
        }?authSource=${process.env.DB_AUTH_DB_NAME}`;
        connect(
            mongouri,
            {
            ...
            (err) => {
                ...
                if (err) throw err;
                console.log(
                    `[DB] Database connected via TCP/IP on port ${process.env.DB_PORT} with TLS encryption`
                );
            }
        );
    };
